// ============================================
// 1. 核心逻辑：新旧界面切换 (独立运行，不依赖配置文件)
// ============================================
document.addEventListener("DOMContentLoaded", () => {

    // 获取新界面的按钮
    const newStartBtn = document.getElementById("newStartButton");

    if (newStartBtn) {
        newStartBtn.addEventListener("click", () => {
            console.log("Button clicked! Starting transition...");

            // 1. 隐藏新界面层
            const newInterfaceLayer = document.getElementById("new-interface-layer");
            if (newInterfaceLayer) {
                newInterfaceLayer.style.opacity = 0;
                newInterfaceLayer.style.transition = "opacity 1s ease-out";
                setTimeout(() => {
                    newInterfaceLayer.style.display = "none";
                }, 1000);
            }

            // 2. 显示原有音乐按钮（如果之前隐藏了）
            const playPauseBtn = document.getElementById("playPauseButton");
            if (playPauseBtn) {
                playPauseBtn.style.display = "block";
                setTimeout(() => playPauseBtn.classList.add("show"), 100);
            }

            // 3. 启动旧版生日动画
            animationTimeline();

            // 4. 智能处理音乐播放
            if (audio) {
                if (audio.paused) {
                    audio.play().then(() => {
                        if (playPauseBtn) playPauseBtn.classList.add('playing');
                        isPlaying = true;
                    }).catch(err => {
                        console.log("自动播放被拦截，需要用户交互");
                    });
                } else {
                    // 如果已经在播放，确保按钮状态同步
                    if (playPauseBtn) playPauseBtn.classList.add('playing');
                    isPlaying = true;
                }
            }
        });
    } else {
        console.error("未找到 id 为 newStartButton 的按钮，请检查 HTML");
    }

    // 初始化音频
    audio = new Audio("music/bgMusic.mp3");
    audio.preload = "auto";
});


// ============================================
// 2. 原有逻辑：读取 JSON 配置 (即使失败也不影响上面)
// ============================================
const fetchData = () => {
    fetch("customize.json")
        .then(data => data.json())
        .then(data => {
            const dataArr = Object.keys(data);
            dataArr.map(customData => {
                if (data[customData] !== "") {
                    if (customData === "imagePath") {
                        const img = document.querySelector(`[data-node-name*="${customData}"]`);
                        if(img) img.setAttribute("src", data[customData]);
                    } else {
                        const el = document.querySelector(`[data-node-name*="${customData}"]`);
                        if(el) el.innerText = data[customData];
                    }
                }
            });
        })
        .catch(err => {
            console.log("读取配置文件失败 (可能是本地打开导致跨域错误)，将使用默认文案:", err);
            // 这里即使报错，也不会影响按钮点击了！
        });
};


// ============================================
// 3. 音频控制逻辑
// ============================================
let audio = null;
let isPlaying = false;

const playPauseButton = document.getElementById('playPauseButton');
if (playPauseButton) {
    playPauseButton.addEventListener('click', () => {
        isPlaying = !isPlaying;
        if (isPlaying) {
            audio.play();
            playPauseButton.classList.add('playing');
        } else {
            audio.pause();
            playPauseButton.classList.remove('playing');
        }
    });
}


// ============================================
// 4. 原有的动画时间轴 (GreenSock GSAP)
// ============================================
const animationTimeline = () => {
    const textBoxChars = document.getElementsByClassName("hbd-chatbox")[0];
    const hbd = document.getElementsByClassName("wish-hbd")[0];

    // 防御性编程：如果没有找到元素，直接返回，防止报错
    if(!textBoxChars || !hbd) return;

    textBoxChars.innerHTML = `<span>${textBoxChars.innerHTML.split("").join("</span><span>")}</span>`;
    hbd.innerHTML = `<span>${hbd.innerHTML.split("").join("</span><span>")}</span>`;

    const ideaTextTrans = { opacity: 0, y: -20, rotationX: 5, skewX: "15deg" };
    const ideaTextTransLeave = { opacity: 0, y: 20, rotationY: 5, skewX: "-15deg" };

    const tl = new TimelineMax();

    tl.to(".container", 0.1, { visibility: "visible" })
        .from(".one", 0.7, { opacity: 0, y: 10 })
        .from(".two", 0.4, { opacity: 0, y: 10 })
        .to(".one", 0.7, { opacity: 0, y: 10 }, "+=2.5")
        .to(".two", 0.7, { opacity: 0, y: 10 }, "-=1")
        .from(".three", 0.7, { opacity: 0, y: 10 })
        .to(".three", 0.7, { opacity: 0, y: 10 }, "+=2")
        .from(".four", 0.7, { scale: 0.2, opacity: 0 })
        .from(".fake-btn", 0.3, { scale: 0.2, opacity: 0 })
        .staggerTo(".hbd-chatbox span", 0.5, { visibility: "visible" }, 0.05)
        .to(".fake-btn", 0.1, { backgroundColor: "#8FE3B6" })
        .to(".four", 0.5, { scale: 0.2, opacity: 0, y: -150 }, "+=0.7")
        .from(".idea-1", 0.7, ideaTextTrans)
        .to(".idea-1", 0.7, ideaTextTransLeave, "+=1.5")
        .from(".idea-2", 0.7, ideaTextTrans)
        .to(".idea-2", 0.7, ideaTextTransLeave, "+=1.5")
        .from(".idea-3", 0.7, ideaTextTrans)
        .to(".idea-3 strong", 0.5, { scale: 1.2, x: 10, backgroundColor: "rgb(21, 161, 237)", color: "#fff" })
        .to(".idea-3", 0.7, ideaTextTransLeave, "+=1.5")
        .from(".idea-4", 0.7, ideaTextTrans)
        .to(".idea-4", 0.7, ideaTextTransLeave, "+=1.5")
        .from(".idea-5", 0.7, { rotationX: 15, rotationZ: -10, skewY: "-5deg", y: 50, z: 10, opacity: 0 }, "+=0.5")
        .to(".idea-5 .smiley", 0.7, { rotation: 90, x: 8 }, "+=0.4")
        .to(".idea-5", 0.7, { scale: 0.2, opacity: 0 }, "+=2")
        .staggerFrom(".idea-6 span", 0.8, { scale: 3, opacity: 0, rotation: 15, ease: Expo.easeOut }, 0.2)
        .staggerTo(".idea-6 span", 0.8, { scale: 3, opacity: 0, rotation: -15, ease: Expo.easeOut }, 0.2, "+=1")
        .staggerFromTo(".baloons img", 2.5, { opacity: 0.9, y: 1400 }, { opacity: 1, y: -1000 }, 0.2)
        .from(".lydia-dp", 0.5, { scale: 3.5, opacity: 0, x: 25, y: -25, rotationZ: -45 }, "-=2")
        .from(".hat", 0.5, { x: -100, y: 350, rotation: -180, opacity: 0 })
        .staggerFrom(".wish-hbd span", 0.7, { opacity: 0, y: -50, rotation: 150, skewX: "30deg", ease: Elastic.easeOut.config(1, 0.5) }, 0.1)
        .staggerFromTo(".wish-hbd span", 0.7, { scale: 1.4, rotationY: 150 }, { scale: 1, rotationY: 0, color: "#ff69b4", ease: Expo.easeOut }, 0.1, "party")
        .from(".wish h5", 0.5, { opacity: 0, y: 10, skewX: "-15deg" }, "party")
        .staggerTo(".eight svg", 1.5, { visibility: "visible", opacity: 0, scale: 80, repeat: (window.innerWidth < 768 ? 1 : 3), repeatDelay: 1.4 }, (window.innerWidth < 768 ? 0.8 : 0.3))
        .to(".six", 0.5, { opacity: 0, y: 30, zIndex: "-1" })
        .staggerFrom(".nine p", 1, ideaTextTrans, 1.2)
        .to(".last-smile", 0.5, { rotation: 90 }, "+=1");

    const replyBtn = document.getElementById("replay");
    if (replyBtn) {
        replyBtn.addEventListener("click", () => {
            tl.restart();
        });
    }
};

// 启动读取配置
fetchData();
